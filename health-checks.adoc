...
1. A liveness check - is the pod running and responding. If it is not then
the cluster will kill and respawn the pod
2. A readiness check - is the server in the pod ready to respond to requests.
If it is not then the cluster will not route requests to it and not continue
with the deployment

The readiness check is helpful because it can prevent requests being sent to
...
code of 200 or 399 for a live or ready pod. For a socket connection, the
...
NOTE: Please delete the current project as we will no longer need it. We are
now actually going to move on to working with code and databases.
